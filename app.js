 ...
 * ฉบับกู้คืนกำลังพล (Fix Blank Staff): ปลดคำสั่งเคลียร์ถาวร + ระบบบังคับเติมรายชื่อทหารเมื่อว่างเปล่า 100%
 */
...
class App {
    ...
    loadData() {
        ...
        if (storedData) {
            ...
            // 🎯 หากหน่วยความจำว่างเปล่า ให้บังคับฉีดกำลังพล DEFAULT เข้าไปทันที
            this.staff = JSON.parse(JSON.stringify(DEFAULT_STAFF)); 
            this.tasks = JSON.parse(JSON.stringify(DEFAULT_TASKS)); 
        }
...
let app;
document.addEventListener('DOMContentLoaded', () => { app = new App(); });            const request = store.get(taskId); request.onsuccess = (e) => resolve(e.target.result); request.onerror = (e) => reject(e);
        });
    }
    deleteAttachment(taskId) {
        return new Promise((resolve, reject) => {
