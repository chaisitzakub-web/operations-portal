 ...
 * ฉบับสมบูรณ์ 100% (Full Version): กู้คืนฟังก์ชันที่หายไปทั้งหมด ระบบกลับมาทำงานได้ 100%
 */
...
class App {
        ...
        if(this.taskPdfInput) {
            this.taskPdfInput.addEventListener('change', (e) => {
                ...
                if(this.pdfUploadStatus) {
                    if (files.length === 0) this.pdfUploadStatus.textContent = 'ไม่มีไฟล์ที่แนบไว้';
                    else if (files.length === 1) this.pdfUploadStatus.textContent = `เลือกไฟล์แล้ว: ${files[0].name}`;
                    else this.pdfUploadStatus.textContent = `เลือกไฟล์แล้ว ${files.length} ไฟล์`;
                }
            });
        }
    ...
    async sendLineAlert(task, actionText) {
        ...
        const targetLineId = member ? member.lineUserId : '';
        ...
        if (this.isCloudMode) {
            try { await fetch('/api/notify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: token, payload: payload }) }); } catch (err) {}
        }
    }
    ...
    async sendMessage(text) {
        ...
        this.scrollToBottomChat();
        if (this.isCloudMode) {
            try { await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(msg) }); } catch (err) {}
        }
    }
    ...
    populateRoleSwitcher() {
        ...
        adminMembers.forEach(member => {
            ...
            opt.value = member.id;
            opt.textContent = member.name + (member.id === 'leader' ? ' (Leader)' : (member.id === 'asst-g3' ? ' (Asst. G3)' : ''));
